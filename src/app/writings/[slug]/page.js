...
const mdxComponents = {
  a: (props) => (
    <a
      ...
      className="text-zinc-900 underline decoration-zinc-900/25 underline-offset-4 hover:decoration-[color:var(--accent)] hover:text-[color:var(--accent)] transition-colors"
    />
  ),
  code: (props) => (
    <code
      {...props}
      className="px-1.5 py-0.5 rounded bg-zinc-900/[0.06] text-zinc-800 font-mono text-[0.9em]"
    />
  ),
  pre: (props) => (
    <pre
  ...
  return (
    <main className="flex min-h-screen flex-col relative overflow-x-hidden" style={{ background: 'var(--bg)' }}>
      ...
      <div className="container max-w-3xl px-6 sm:px-10 py-4 mx-auto mt-24 relative z-10">
        <Link
          href="/writings"
          className="inline-flex items-center gap-1.5 text-zinc-500 hover:text-zinc-900 text-sm mt-4 mb-10 transition-colors"
        >
          <span>←</span> Back to writings
        </Link>

        <header className="pb-8 border-b border-zinc-900/10">
          {frontmatter.project && (
            <Link
              href="/#projects"
            ...
            >
              <span className="text-zinc-500">Related project:</span>
              <span className="font-medium text-zinc-900">{frontmatter.project}</span>
          ...
          <h1 className="serif text-4xl sm:text-5xl font-normal text-zinc-900 tracking-tight leading-tight mb-4">
            {frontmatter.title}
          </h1>
        ...
        <article
          className="prose prose-zinc max-w-none py-10
            ...
            prose-blockquote:border-l-zinc-900/20 prose-blockquote:text-zinc-600 prose-blockquote:not-italic
            prose-hr:border-zinc-900/10
            prose-img:rounded-lg"
        >
          <MDXRemote source={content} components={mdxComponents} />
        </article>

        <div className="border-t border-zinc-900/10 pt-8 pb-16">
          <Link
            href="/writings"
