...
export default function CurrentlyCard() {
  return (
    <motion.div
      ...
      className="relative w-full rounded-2xl overflow-hidden border border-zinc-900/[0.08] dark:border-zinc-100/[0.08]
        shadow-[0_12px_40px_-16px_rgba(0,0,0,0.12)] dark:shadow-[0_12px_40px_-16px_rgba(0,0,0,0.45)]"
      style={{
        // Cream paper with very subtle noise texture (or warm-tinted dark in dark mode)
        background: `
          radial-gradient(ellipse at top right, rgba(232,130,92,0.04), transparent 50%),
    ...
    >
      ...
      <div className="relative pt-8 pb-2 flex items-center justify-center gap-3">
        <span className="h-px w-12" style={{ background: 'var(--ink-4)' }} />
        <span className="text-zinc-500 dark:text-zinc-400 text-sm leading-none">✦</span>
        <span className="h-px w-12" style={{ background: 'var(--ink-4)' }} />
      </div>

      {/* Header */}
      <div className="relative px-8 pb-6 text-center">
        <p className="serif-italic text-xs text-zinc-500 dark:text-zinc-400 mb-2 tracking-wider">
          Field Notes
        </p>
        <h3 className="serif-italic text-[32px] leading-tight font-normal text-zinc-900 dark:text-zinc-100 mb-2">
          What's on my desk.
        </h3>
        <p className="text-[10px] uppercase tracking-[0.25em] text-zinc-400 dark:text-zinc-500 tabular-nums">
          {formatRomanDate()}
        </p>
      </div>
          ...
          >
            ...
            {i > 0 && (
              <div className="flex items-center justify-center gap-3 py-6">
                ...
                <span className="text-zinc-300 dark:text-zinc-600 text-xs leading-none select-none">✦</span>
                <span className="h-px w-8" style={{ background: 'var(--line)' }} />
              </div>
            )}
            ...
            <div className={i === 0 ? 'pt-2 pb-2' : 'pb-2'}>
              ...
              <div className="flex items-baseline gap-2.5 mb-2.5">
                <span
                  className="serif-italic text-base text-zinc-400 dark:text-zinc-500 leading-none w-7 shrink-0"
                  style={{ color: s.active ? 'var(--accent)' : undefined }}>
                  {s.roman}
                </span>
                <span
                  className="serif-italic text-base text-zinc-500 dark:text-zinc-400 leading-none"
                  style={{ color: s.active ? 'var(--accent)' : undefined }}>
                  {s.label}
                </span>
                {s.active && (
              ...
              {/* Title */}
              <h4 className="serif text-[20px] font-normal text-zinc-900 dark:text-zinc-100 leading-snug mb-2 pl-[2.4rem]">
                {s.title}
              </h4>

              {/* Body */}
              <p className="serif-italic text-[14px] text-zinc-600 dark:text-zinc-300 leading-[1.75] pl-[2.4rem]">
                {s.body}
              </p>
              ...
              {s.quote && (
                <div className="mt-3 ml-[2.4rem] -translate-x-[2.4rem] pl-[2.4rem]">
                  <blockquote
                    className="serif-italic text-[14px] text-zinc-700 dark:text-zinc-200 leading-relaxed
                      border-l-2 pl-3.5"
                    style={{ borderColor: 'var(--accent)' }}>
                    {s.quote}
                  </blockquote>
                  {s.attribution && (
                    <p className="text-[11px] text-zinc-400 dark:text-zinc-500 pl-3.5 mt-1.5 tracking-wide">
                      {s.attribution}
                    </p>
                  )}
                </div>
              ...
              {s.href && (
                <a
                  ...
                  className="serif-italic inline-flex items-center gap-1 mt-3 ml-[2.4rem] text-[13px] text-zinc-700 dark:text-zinc-300
                    hover:text-[color:var(--accent)] dark:hover:text-[color:var(--accent)] transition-colors group/cta border-b border-zinc-300 dark:border-zinc-600
      ...
      <div className="relative px-8 pb-7 pt-2 flex items-end justify-between">
        <p className="serif-italic text-[12px] text-zinc-400 dark:text-zinc-500 leading-relaxed">
          Always under<br />construction.
        </p>
        <div className="flex flex-col items-end relative">
          <span className="signature text-[28px] text-zinc-800 dark:text-zinc-200 leading-none">
            Aaron
          </span>
          {/* Stamp-like date */}
          <div
      ...
      <div className="absolute bottom-2 right-3 serif-italic text-[10px] text-zinc-300 dark:text-zinc-600 select-none">
        — i —
      </div>
    </motion.div>
  )
